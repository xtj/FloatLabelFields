...
@IBDesignable class FloatLabelTextView: UITextView {
	...
	private func setup() {
		...
		if !isIB {
			let nc = NSNotificationCenter.defaultCenter()
      nc.addObserver(self, selector: #selector(layoutSubviews), name:UITextViewTextDidChangeNotification, object:self)
			nc.addObserver(self, selector: #selector(layoutSubviews), name:UITextViewTextDidBeginEditingNotification, object:self)
			nc.addObserver(self, selector: #selector(layoutSubviews), name:UITextViewTextDidEndEditingNotification, object:self)
		}
	}
